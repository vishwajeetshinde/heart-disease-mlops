...
class TestDataCleaning:
    ...
    def test_age_range(self):
        ...
        df = pd.read_csv("notebooks/data/heart_cleaned.csv")
        assert (df["age"] >= 0).all() and (
            df["age"] <= 120
        ).all(), "Age should be between 0 and 120"
...
class TestDataStatistics:
    ...
    def test_no_constant_features(self):
        ...
        for col in df.columns:
            ...
            assert (
                n_unique > 1
            ), f"Feature {col} is constant (only {n_unique} unique value)"
...
class TestDataTransformations:
    ...
    def test_data_can_be_split(self):
        ...
        assert (
            X.shape[0] == y.shape[0]
        ), "Features and target should have same number of rows"
    ...
    def test_train_test_split_possible(self):
        ...
        assert len(X_train) + len(X_test) == len(
            X
        ), "Split should account for all samples"
    ...
    def test_data_normalization_possible(self):
        ...
        assert not np.isinf(
            X_scaled
        ).any(), "Normalized data should not contain infinity"
...
class TestDataIntegrity:
    ...
    def test_outlier_detection(self):
        ...
        for col in ["age", "trestbps", "chol", "thalach", "oldpeak"]:
            ...
            assert (
                outlier_ratio < 0.1
            ), f"Feature {col} has too many outliers: {outlier_ratio:.2%}"
