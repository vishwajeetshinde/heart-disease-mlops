...
def invalid_data():
    ...
    return pd.DataFrame({"wrong_col1": [1, 2, 3], "wrong_col2": [4, 5, 6]})
...
class TestDataValidation:
    ...
    def test_data_shape(self, sample_data):
        ...
        assert sample_data.shape == (
            5,
            13,
        ), "Sample data should have 5 rows and 13 columns"

    def test_data_columns(self, sample_data, sample_columns):
        """Test if data has all required columns"""
        assert (
            list(sample_data.columns) == sample_columns
        ), "Data columns don't match expected columns"
    ...
    def test_data_ranges(self, sample_data):
        ...
        assert (sample_data["age"] >= 0).all() and (
            sample_data["age"] <= 120
        ).all(), "Age should be between 0-120"
...
class TestPreprocessingPipeline:
    ...
    def test_preprocess_input_shape(self, sample_data):
        ...
        assert (
            X_scaled.shape == sample_data.shape
        ), "Scaled data should have same shape as input"
    ...
    def test_preprocess_output_type(self, sample_data):
        ...
        assert isinstance(
            X_scaled, np.ndarray
        ), "Preprocessed output should be numpy array"
...
class TestModelLoading:
    ...
    def test_logistic_regression_exists(self):
        ...
        model_path = "notebooks/models/logistic_regression.pkl"
    ...
    def test_random_forest_exists(self):
        ...
        model_path = "notebooks/models/random_forest.pkl"
        assert os.path.exists(
            model_path
    ...
    def test_logistic_regression_load(self):
        ...
        model = joblib.load("notebooks/models/logistic_regression.pkl")
...
class TestModelPredictions:
    ...
    def test_predict_proba_shape(self, sample_data):
        ...
        assert probabilities.shape == (
            len(sample_data),
            2,
        ), "Probabilities should be (n_samples, 2)"
    ...
    def test_batch_prediction_consistency(self, sample_data):
        ...
        assert np.array_equal(
            batch_predictions, individual_predictions
...
class TestModelComparison:
    ...
    def test_models_have_same_classes(self):
        ...
        assert np.array_equal(
            lr_model.classes_, rf_model.classes_
...
class TestEdgeCases:
    ...
    def test_missing_column_fails(self, sample_columns):
        ...
        incomplete_df = pd.DataFrame(
            {col: [1] for col in sample_columns[:-1]}
        )  # Missing last column
        with pytest.raises(Exception):
    ...
    def test_extreme_values(self, sample_columns):
        ...
        assert prediction[0] in [
            0,
            1,
        ], "Should handle extreme values and return valid prediction"
